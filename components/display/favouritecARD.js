...
const FavoriteCard = ({ favoriteItem, viewMode, toggleFavorite }) => {
  ...
  if (viewMode === "grid") {
    ...
    if (isLoading && !movieData.title && !movieData.name) {
      return (
        <div className="bg-muted border border-border rounded-2xl aspect-[2/3] flex flex-col items-center justify-center gap-4 animate-pulse">
    ...
    return (
      <motion.div
        layout
        className="group relative rounded-2xl overflow-hidden bg-card border border-border isolate"
      >
        <Link
          href={getLink()}
          title={renderTitle()}
        ...
        >
          ...
          <div className="absolute top-3 left-3 right-3 flex justify-between items-start z-10">
            <div className="flex items-center gap-2">
              <div className="px-2 py-1 bg-background/40 backdrop-blur-md border border-border rounded text-[10px] font-mono text-foreground uppercase tracking-wider flex items-center gap-1.5">
                {isTV ? (
              ...
              {movieData.vote_average > 0 && (
                <div className="px-2 py-1 bg-background/40 backdrop-blur-md border border-border rounded text-[10px] font-mono text-accent-foreground font-bold flex items-center gap-1">
            ...
            <button
              ...
              className="p-2 rounded-full bg-background/40 backdrop-blur-md border border-border text-foreground hover:bg-foreground/10 hover:scale-110 transition-all active:scale-95"
            >
              <Heart
                size={16}
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 z-20 flex flex-col justify-end">
            ...
            {/* Title */}
            <h3 className="text-lg md:text-xl font-black text-foreground leading-tight tracking-tight mt-2 line-clamp-2 drop-shadow-lg group-hover:line-clamp-none transition-all">
              {renderTitle()}
            </h3>
            ...
            <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-out">
              <div className="overflow-hidden">
                <p className="text-xs text-muted-foreground line-clamp-3 mt-3 leading-relaxed font-sans">
                  {movieData.overview ||
                    "No overview available for this title."}
                </p>

                {/* CTA Button */}
                <div className="flex items-center gap-2 text-xs font-bold text-foreground uppercase tracking-wider mt-4 group/btn">
