...
const SearchDisplay = ({ media }) => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <div className="hidden md:block mb-6 p-4 bg-card border border-border rounded-lg shadow-md">
        <div className="flex flex-wrap gap-4 items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-muted-foreground text-sm">Min Rating:</label>
              <input
                type="number"
                min="0"
                max="10"
                ...
                className="w-20 bg-muted text-foreground rounded px-3 py-2 text-sm border border-border focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none"
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                className="bg-muted text-foreground rounded px-3 py-2 text-sm border border-border focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none"
                value={yearFilter}
                onChange={(e) => {
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <button
                className="p-2 bg-muted rounded hover:bg-accent border border-border group relative"
                onClick={() =>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 text-sm bg-muted hover:bg-accent rounded border border-border transition duration-200 text-muted-foreground hover:text-foreground flex items-center gap-1"
            >
              <X className="w-4 h-4" />
              Reset
            </button>
      ...
      <div className="md:hidden mb-4 flex justify-between items-center">
        ...
        <button
          ...
          className="p-2 bg-card border border-border rounded-lg shadow-md flex items-center gap-1 text-sm"
        >
      ...
      {showMobileFilters && (
        <div className="md:hidden mb-6 p-4 bg-card border border-border rounded-lg shadow-md animate-slideDown">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">Minimum Rating:</label>
              <div className="flex items-center gap-2">
                <input
                  type="range"
                  min="0"
                  ...
                  disabled={showAllResults}
                />
                <span className="text-foreground bg-muted px-2 py-1 rounded min-w-[40px] text-center">
                  {minRating}
                </span>
              </div>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-muted-foreground text-sm">Year:</label>
              <select
                className="w-full bg-muted text-foreground rounded px-3 py-2 text-sm border border-border focus:border-primary focus:outline-none"
                value={yearFilter}
                onChange={(e) => {
                  setYearFilter(e.target.value);
                  setCurrentPage(1);
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-2">
                <select
                  className="flex-grow bg-muted text-foreground rounded px-3 py-2 text-sm border border-border focus:border-primary focus:outline-none"
                  value={sortField}
                  onChange={(e) => {
                    setSortField(e.target.value);
                    // When changing sort field, reset to appropriate default direction
                ...
                <button
                  className="p-2 bg-muted rounded hover:bg-accent border border-border"
                  onClick={() =>
                    setSortDirection(sortDirection === "asc" ? "desc" : "asc")
                  }
                >
            ...
            <div className="flex justify-between pt-2">
              <button
                onClick={resetFilters}
                className="px-4 py-2 bg-muted hover:bg-accent rounded border border-border text-muted-foreground hover:text-foreground text-sm"
              >
                Reset All
              </button>
              <button
                onClick={() => setShowMobileFilters(false)}
                className="px-4 py-2 bg-primary hover:opacity-90 rounded text-primary-foreground text-sm"
              >
                Apply Filters
              </button>
            </div>
      ...
      {paginatedMedia.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-8 max-w-md mx-auto">
          <div className="flex flex-col items-center">
            ...
            {activeFiltersCount() > 0 && (
              <button
                onClick={resetFilters}
                className="mt-4 px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm hover:opacity-90 transition"
              >
                Reset All Filters
              </button>
            )}
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-2 mt-4 mb-8 bg-card border border-border rounded-full px-2 py-1 shadow-md w-fit mx-auto">
            <button
              className="p-2 rounded-full hover:bg-accent disabled:opacity-50 disabled:hover:bg-transparent transition duration-200"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center mb-8">
              <input
                ...
                className="w-16 bg-muted text-foreground rounded px-3 py-2 text-sm border border-border focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none text-center"
              />
              <button
                onClick={() => {
                ...
                className="ml-2 px-3 py-2 bg-primary text-primary-foreground hover:opacity-90 rounded text-sm"
              >
                Go
              </button>
            </div>
