...
const TvCards = (props) => {
  ...
  // Rating and release year (for TV shows)
  const rating = TvCard.vote_average?.toFixed(1) || "N/A";
  const releaseDate = TvCard.first_air_date; // Assuming the air date is available
  ...
  // Check if the TV show is favorited when component mounts
  useEffect(() => {
    const favorites = JSON.parse(localStorage.getItem("favorites")) || [];
    const isTvFavorited = favorites.some((item) => item.id === TvCard.id);
    setIsFavorite(isTvFavorited);
  }, [TvCard.id]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 text-white">
        <h3 className="text-lg font-semibold truncate mb-1">{TvCard.name}</h3>
