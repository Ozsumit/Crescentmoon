...
const EmptyState = () => (
  <tr>
    <td
      colSpan={5}
      className="py-16 text-center text-neutral-400 dark:text-neutral-600 text-sm"
    >
...
const SourceManagement = ({ initialSources }) => {
  ...
  const handleSeed = async () => {
    if (
      confirm(
        "This will add default video sources if they don't exist. Continue?",
      )
    ) {
            ...
            >
              ...
              <Field label="Features (comma-separated)" span2>
                <input
                  className={inputCls}
                  type="text"
                  placeholder="Fast, Multi-Language"
              ...
              <div className="md:col-span-2 flex items-center justify-between pt-2">
                <label className="flex items-center gap-2.5 cursor-pointer select-none">
                  ...
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${formData.active ? "translate-x-4" : "translate-x-0"}`}
                    />
                  </div>
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={handleCancel}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm font-semibold hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all"
                  >
                    <X size={14} /> Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold transition-all shadow-sm"
                  >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-neutral-100 dark:border-neutral-800">
                {["Priority", "Source", "Features", "Status", ""].map(
                  (h, i) => (
                    <th
                      key={i}
                      className={`px-5 py-4 text-[10px] font-semibold uppercase tracking-[0.15em] text-neutral-400 dark:text-neutral-500 ${i === 4 ? "text-right" : ""}`}
                    >
                      {h}
                    </th>
                  ),
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-sm text-neutral-900 dark:text-neutral-100 leading-tight">
                            {s.name}
                          </div>
                          <div className="text-[10px] text-neutral-400 dark:text-neutral-600 truncate max-w-[200px] font-mono mt-0.5">
                            {s.url}
                          </div>
                        </div>
                      </div>
